              ...
              {products?.map((product: Product) => {
                return (
                  <div key={product.id} className="flex flex-col items-start justify-center gap-5">
                    <Image src={`/assets/${product.image}`} alt={product.name} width={0} height={0} sizes="100vw" style={{ width: "100%" }} className="aspect-square rounded-t-md object-cover shadow-lg hover:cursor-pointer" />
                    ...
                    <div className="w-full">
                      ...
                      <Link href={`/product/${product.id}`}>
                        <Button size={"sm"} className="mt-5 w-full bg-brown-900 hover:bg-brown-800 active:bg-brown-700">
                          Buy Now
                        </Button>
                      </Link>
                    </div>
